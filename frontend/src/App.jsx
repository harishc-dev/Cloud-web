...
export default function App() {
  ...
  useEffect(() => {
    const storedUsername = localStorage.getItem('cloudUsername') || sessionStorage.getItem('cloudUsername');
    if (storedUsername) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-[16rem_1fr] gap-3 md:gap-6">
          ...
          <main className="space-y-3 md:space-y-4">
            ...
            <div className={`${neon.border} ${neon.glow} rounded-2xl p-4 bg-slate-900/50 flex items-center justify-between`}>
              <div className="text-sm text-slate-400">Not all clouds rain — some protect.</div>
              <a
                href="https://harishc-dev.me"
